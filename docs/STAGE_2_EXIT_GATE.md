...
| Real User polling/context uses OAuth from Keychain | Not proven | Bundle parsing and OAuth v3 refresh response handling pass synthetically; no production transport, atomic Keychain rotation, or scheduler |
...
| Real Feishu reply succeeds | Not proven | Durable dispatch, credential parsing, and refresh semantics are covered synthetically; no production HTTP client, atomic rotation, composed adapter, or live account |
...
production token transport and Keychain rotation, live Feishu API semantics,
