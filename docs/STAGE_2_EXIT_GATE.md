...
| User edits and approves in the product UI | Editing, exact preview, and one-time approval implemented; execution not implemented | The Inbox creates sequential local Draft revisions, marks one `ready_for_review`, persists the exact User-identity reply preview, opens a fixed 15-minute approval window, and records approval, rejection, cancellation, or expiration with restart-repairable Audit. It re-displays the exact account, identity, message target, risk, expiration, and content. Approval is not consumed and no send control is exposed. |
