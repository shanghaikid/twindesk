...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, restart-safe app-bound registered-loopback configuration and capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; no browser/Cordis lifecycle, Settings UI/runtime loading, polling HTTP adapter, scheduler, or recovery UI |
