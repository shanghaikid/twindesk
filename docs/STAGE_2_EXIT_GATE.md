...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, literal-loopback capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; no browser/Cordis lifecycle, registered redirect configuration, polling HTTP adapter, scheduler, or recovery UI |
