...
| Exclusive Feishu Host ownership | Contract passed | A kernel-backed loopback lease excludes real competing processes, releases after `SIGKILL`, and now owns durable User rotation, one full synthetic Workbench reply operation, and the complete injected-client polling lifecycle |
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, default-path restart-loaded app-bound registered-loopback configuration and capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, exact User scope probing, and a restart-safe atomic polling scheduler pass synthetically. The scheduler reloads every durable page cursor, holds the exclusive lease, applies bounded retry, and stops on authorization or storage failure. Live authorization and reauthorization acceptance, existing-identity replacement, credential repair/removal, Cordis activation, a concrete OAuth-backed polling HTTP adapter, and live polling remain open. |
