...
| User edits and approves in the product UI | Editing and proposal preview implemented; approval not implemented | The Inbox creates sequential local Draft revisions, marks one `ready_for_review`, and can persist an exact User-identity Feishu reply preview with a user-attributed Audit record. The UI displays the exact account, identity, message target, risk, and content. Approval and execution remain unavailable. |
...
a credential-healthy production provider, and implements exact approval UI
