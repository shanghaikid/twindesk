...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, default-path restart-loaded app-bound registered-loopback configuration and capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically. The default Web launcher now injects minimized Settings, create-only User identity bootstrap, a CSRF-protected callback/scope editor, separate memory-only initial authorization and recovery-gated reauthorization entries, and an identifier-free recovery reader. Reauthorization holds one lease through callback, exchange, a durable-before-Keychain version 3 replacement reservation, verified replacement, and settlement; interruption or uncertain write remains reconciliation-required across restart. An explicit lease-held local action compares the exact Keychain bundle with that journal and settles strictly newer evidence without OAuth, refresh, or Keychain write; its request/result business Audit is durable and restart-repairable without another Keychain read. Live authorization and reauthorization acceptance, existing-identity replacement, credential repair/removal, Cordis activation, polling HTTP adapter, and scheduler remain open. |
...
| Model-backed Draft and Harness trace are linked | Persistence protocol passed; runtime not composed | Work Hub can durably link one bounded `editing` Draft and Audit to an opaque WorkItem -> Session -> Run chain, repair an interrupted Audit without another model Run, and preserve store separation. The acceptance Draft remains deterministic with `modelInvocation: false`; production Harness invocation and Session-flush composition are not yet bound. |
...
reply operation, composes production Harness invocation and Session flush with
the model Draft linkage, and implements product Draft and approval UI plus an
authorized live-account acceptance run. Stage 3 must not rely on a claimed
