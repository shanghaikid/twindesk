...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, default-path restart-loaded app-bound registered-loopback configuration and capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; the blocked-state runtime now also reconstructs production collaborators from Settings plus a supplied concrete journal and holds one lease through callback, exchange, verified replacement, and settlement. The default Web launcher injects minimized Settings, create-only User identity bootstrap, a CSRF-protected callback/scope editor, an explicit memory-only initial authorization entry, and an identifier-free recovery-state reader that blocks unresolved retries, but it does not yet wire the hosted reauthorization runtime or expose a recovery action/API. Live authorization acceptance, existing-identity replacement, Cordis activation, polling HTTP adapter, scheduler, and reauthorization/reconciliation UI also remain open. |
