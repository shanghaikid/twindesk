...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, verified initial and lease-held blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; no hosted redirect listener, polling HTTP adapter, scheduler, or recovery UI |
