...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, default-path restart-loaded app-bound registered-loopback configuration and capture through lease-held verified initial persistence, blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; the fixed private macOS root constructs the secret-free recovery journal, and the default Web launcher injects minimized Settings, create-only User identity bootstrap, a CSRF-protected callback/scope editor, an explicit memory-only initial authorization entry, and an identifier-free recovery-state reader that blocks unresolved retries, but no recovery action, live authorization acceptance, existing-identity replacement, Cordis activation, polling HTTP adapter, scheduler, or hosted reauthorization/reconciliation UI exists |
