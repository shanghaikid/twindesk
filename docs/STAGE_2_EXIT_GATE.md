...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, bundle parsing, post-exchange `open_id` verification through the bounded production Fetch client, and durable single-Host rotation pass synthetically; no verified initial Keychain persistence, hosted redirect listener, exclusive runtime lease, live composition, or scheduler |
...
| Real Feishu reply succeeds | Not proven | Durable dispatch and single-Host credential rotation are covered synthetically; no exclusive runtime lease, reply HTTP client, composed adapter, or live account |
...
would incorrectly imply support for authorization-code/PKCE exchange composed
with verified initial persistence, exclusive Connector ownership, composed live
Feishu API semantics, callback or polling lifecycle, interactive approval, and
a real external receipt.
