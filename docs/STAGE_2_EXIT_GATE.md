...
| Real User polling/context uses OAuth from Keychain | Not proven | Code/PKCE exchange, literal-loopback capture, verified initial and lease-held blocked-state replacement, durable reply-path rotation, and exact User scope probing pass synthetically; no Workbench listener-to-persistence composition, browser lifecycle, polling HTTP adapter, scheduler, or recovery UI |
