...
| Real User polling/context uses OAuth from Keychain | Not proven | Bundle parsing and the bounded OAuth v3 HTTP transport pass synthetically; no atomic Keychain rotation, live composition, or scheduler |
...
| Real Feishu reply succeeds | Not proven | Durable dispatch, credential parsing, and refresh semantics are covered synthetically; no reply HTTP client, atomic rotation, composed adapter, or live account |
...
would incorrectly imply support for authorization-code identity verification,
atomic Keychain rotation, composed live Feishu API semantics, callback or
