...
| User edits and approves in the product UI | Editing implemented; approval not implemented | The Inbox creates sequential local Draft revisions and can mark one `ready_for_review` with a user-attributed Audit record. This grants no proposal, approval, or execution authority; exact proposal and approval UI remain open. |
