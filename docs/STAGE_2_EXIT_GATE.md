...
| Real User polling/context uses OAuth from Keychain | Not proven | Bundle parsing, post-exchange `open_id` verification through the bounded production Fetch client, and durable single-Host rotation pass synthetically; no code/PKCE exchange, verified initial Keychain persistence, exclusive runtime lease, live composition, or scheduler |
