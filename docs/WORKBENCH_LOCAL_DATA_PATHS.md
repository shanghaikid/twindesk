...
data location for the macOS MVP. It does not touch disk. The fixed root is:
...
record covers only the two non-secret Feishu Settings documents:
...
Neither document may contain a client secret, OAuth code, verifier, token,
cookie, or private key; credentials remain in the system Keychain.

## Verification and Remaining Work

Synthetic tests verify the exact macOS layout, deep immutability, private
directory modes, restart recovery through newly constructed stores, absence of
secret-like fields, unsupported and aliased path rejection, hostile accessor
avoidance, and refusal to traverse linked or publicly accessible directories.
The contract covers observed preparation-time state; it is not a filesystem
lock against a same-user process that mutates directories after the stores are
returned.
...
- Connectors/Settings UI editing and presentation-safe local APIs;
- explicit disconnect, configuration deletion, OAuth revocation, and Keychain
  deletion as separate user actions;
- product paths for SQLite business data, Harness Sessions, and secret-free
  OAuth recovery journals;
