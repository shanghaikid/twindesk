...
export interface WorkbenchFeishuBotIdentityCreate {
  readonly version: 1
...
export interface WorkbenchFeishuBotIdentityBootstrapper {
  create(value: unknown): Promise<void>
}

export interface WorkbenchFeishuBotIdentityBootstrapperOptions {
...
function invalid(): TypeError {
  return new TypeError('The Workbench Feishu Bot identity creation is invalid.')
}
...
function readOptions(value: unknown): WorkbenchFeishuBotIdentityBootstrapperOptions {
...
function createData(value: unknown): WorkbenchFeishuBotIdentityCreate {
...
/** Create one Bot identity and opaque Keychain reference without accepting a credential. */
export function createWorkbenchFeishuBotIdentityBootstrapper(
  optionsValue: WorkbenchFeishuBotIdentityBootstrapperOptions,
): WorkbenchFeishuBotIdentityBootstrapper {
  ...
  return Object.freeze({
    async create(value: unknown): Promise<void> {
      ...
      const operation = pendingCreate.then(async () => {
        ...
        if (existing?.bot !== undefined) throw invalid()
        if (
        ...
        await options.identityStore.write({
          ...
          bot: {
            ...
            credentialReference: {
              ...
              schemaVersion: 1,
              id: `secret-ref:feishu-bot-app-${opaqueSuffix()}`,
              store: 'system_keychain',
              purpose: 'connector_app_credential',
            },
          },
          ...(existing?.user === undefined ? {} : { user: existing.user }),
        })
      })
