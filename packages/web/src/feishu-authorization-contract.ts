export type FeishuAuthorizationRecovery =
  | 'configure_settings'
...
export type FeishuAuthorizationSnapshot =
  ...
  | Readonly<{
      ...
      connectorId: 'feishu'
      state: 'failed'
      recovery: FeishuAuthorizationRecovery
    }>

type UnknownRecord = Readonly<Record<string, unknown>>
const RECOVERIES: readonly FeishuAuthorizationRecovery[] = Object.freeze([
  'configure_settings',
  'correct_configuration',
  'use_reauthorization',
  'reauthorize',
...
function invalid(): never {
  throw new Error('Local API returned an invalid Feishu authorization response.')
}
...
function authorizationUrls(record: UnknownRecord): Readonly<{
  authorizationUrl: string
  redirectUri: string
}> {
  if (
...
/** Parse the exact memory-only authorization state before server or browser use. */
export function parseFeishuAuthorizationSnapshot(value: unknown): FeishuAuthorizationSnapshot {
  ...
  if (base.state === 'waiting') {
    return Object.freeze({
      ...
      connectorId: 'feishu',
      state: 'waiting',
      ...authorizationUrls(base),
    })
  }
  if (
    base.state === 'failed' &&
    typeof base.recovery === 'string' &&
    RECOVERIES.includes(base.recovery as FeishuAuthorizationRecovery)
  ) {
    return Object.freeze({
      version: 1,
      connectorId: 'feishu',
      state: 'failed',
      recovery: base.recovery as FeishuAuthorizationRecovery,
    })
  }
  return invalid()
}
